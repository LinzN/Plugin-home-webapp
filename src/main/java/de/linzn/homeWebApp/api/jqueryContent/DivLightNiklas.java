...
public class DivLightNiklas implements IResponseHandler {
    ...
    public IHtmlTemplate buildResponse(List<String> inputList) {
        ...
        DeviceStatus status = new DeLockSwitch("niklas").getStatus();
        ...
        if (status == DeviceStatus.ENABLED) {
            stringBuilder.append("<ok><button onclick=\"$.post('" + url + "/post_light-control/niklas/0')\" class=\"far fa-lightbulb tbutton\"></button></ok>");
        } else if (status == DeviceStatus.DISABLED) {
            stringBuilder.append("<button onclick=\"$.post('" + url + "/post_light-control/niklas/1')\" class=\"far fa-lightbulb tbutton\"></button>");
        } else {
